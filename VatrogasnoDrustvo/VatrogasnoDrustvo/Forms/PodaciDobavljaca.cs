...
namespace VatrogasnoDrustvo.InputForms
{
    ...
    public partial class PodaciDobavljaca : Form
    {
        ...
        private void updateDobavljac()
        {
            ...
            try
            {
                //MessageBox.Show(new Sender().Send(tvrtka, "http://46.101.122.250/updateDobavljac.php"));
                var response = JsonConvert.DeserializeObject<Dictionary<string, object>>(new Sender().Send(tvrtka, "http://46.101.122.250/updateDobavljac.php"));
        ...
        private void createDobavljac()
        {
            ...
            try
            {
                var response = JsonConvert.DeserializeObject<Dictionary<string, object>>(new Sender().Send(tvrtka, "http://46.101.122.250/insertDobavljac.php"));
                if (bool.Parse(response["passed"].ToString()))
                {
